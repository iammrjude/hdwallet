...
contract swvlTokenSale is Crowdsale, CappedCrowdsale, TimedCrowdsale, AllowanceCrowdsale {
    uint public investorMinCap  = 2000000000000000; // 0.002 Ether
    uint public investorHardCap = 5000000000000000000; // 5 Ether
    mapping(address => uint256) public contributions; // private
    //mapping(address => uint256) private _caps;
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view {
        super._preValidatePurchase(beneficiary, weiAmount);
        uint _existingContribution = contrubutions(beneficiary);
        uint _newContribution = _existingContribution.add(weiAmount);
        // solhint-disable-next-line max-line-length
